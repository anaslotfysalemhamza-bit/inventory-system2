...
// ========== WebSocket Server ==========
// في Vercel، WebSocket مش مدعوم بشكل كامل، لكن هنضيفه لو شغال محلياً
let wss = null;
let clients = [];
    ...
    html: `
            <div dir="rtl" style="font-family: 'Cairo', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8fafc; border-radius: 20px;">
                ...
                <div style="background: white; padding: 30px; border-radius: 16px; text-align: center;">
                    ...
                    ${isApproved ? `<a href="${process.env.FRONTEND_URL || "https://inventory-system.vercel.app"}/login.html" style="display: inline-block; background: #4361ee; color: white; padding: 12px 30px; border-radius: 30px; text-decoration: none; margin-top: 20px;">تسجيل الدخول</a>` : ""}
                </div>
            </div>
        `,
  };
...
app.get("/api/customer-debts-all", async (req, res) => {
  try {
    const [rows] = await db.query(`
            ...
            FROM customer_debts cd            LEFT JOIN customers c ON cd.customerId = c.id
